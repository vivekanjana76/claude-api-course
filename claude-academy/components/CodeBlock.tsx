...
function highlight(code: string, lang: string): React.ReactNode[] {
  ...
  const kw =
    /\b(import|from|def|return|if|else|elif|for|while|in|with|as|class|try|except|True|False|None|async|await|const|let|var|function|new|null|true|false|while|break|continue)\b/g;
  return lines.map((line, i) => {
    ...
    while ((m = strRegex.exec(codePart)) !== null) {
      ...
      segs.push(
        <span key={k++} className="text-sage">
          {m[0]}
        </span>,
      );
    ...
    return (
      <div key={i} className="table-row">
        <span className="table-cell pr-4 select-none text-right text-ink-faint/50 text-xs w-8">
          {i + 1}
        </span>
...
function kwSplit(text: string, kw: RegExp): React.ReactNode[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    out.push(
      <span key={k++} className="text-clay-light font-medium">
        {m[0]}
      </span>,
    );
    last = m.index + m[0].length;
...
export function CodeBlock({
  ...
  return (
    <figure className="my-7">
      <div className="rounded-xl overflow-hidden border border-ink/10 bg-[#1F1D18] shadow-md">
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/10">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-xs text-cream/50 font-mono uppercase tracking-wide">
              {lang}
            </span>
          </div>
          <button
        ...
        <pre className="p-4 overflow-x-auto text-[13px] leading-relaxed text-cream-100 font-mono thin-scroll">
