    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {timed && (
            <span
              className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-sm font-medium tabular-nums ${
                lowTime ? "bg-cream-200 text-slateblue" : "text-ink-muted"
              }`}
            >
          ...
          <button
            onClick={onExit}
            className="flex items-center gap-1 rounded-lg px-2 py-1 text-sm text-ink-muted transition-colors hover:bg-cream-200 hover:text-ink"
          >
            <X size={14} /> Abandon
          </button>
        </div>
      ...
      <div className="mb-5 flex flex-wrap gap-1.5">
        {questions.map((_, n) => {
          const state = flagged[n]
            ? "border-ochre bg-cream-200 text-ochre"
            : picks[n] !== null
              ? "border-clay/50 bg-clay-50 text-clay-dark"
              : "border-cream-300 bg-cream-50 text-ink-faint";
          return (
            <button
              key={n}
              onClick={() => setI(n)}
              ...
              className={`h-7 w-7 rounded-md border text-[0.7rem] font-medium tabular-nums transition-colors ${state} ${
                n === i ? "ring-2 ring-ink ring-offset-1 ring-offset-cream" : ""
              }`}
            >
              {n + 1}
            </button>
      ...
      <div className="rounded-2xl border border-cream-300 bg-cream-50 p-6 shadow-sm">
        <div className="mb-4 flex items-start justify-between gap-4">
          <p className="font-serif text-lg leading-snug text-ink">{q.q}</p>
          <button
            onClick={() =>
              setFlagged((f) => {
                const next = [...f];
            ...
            className={`shrink-0 rounded-lg p-2 transition-colors ${
              flagged[i]
                ? "bg-cream-200 text-ochre"
                : "text-ink-faint hover:bg-cream-200 hover:text-ink"
            }`}
          >
            <Flag size={15} />
          </button>
          ...
          {q.options.map((opt, oi) => (
            <button
              ...
              className={`flex w-full items-start gap-3 rounded-xl border px-4 py-3 text-left text-[0.94rem] transition-colors ${
                picks[i] === oi
                  ? "border-clay/60 bg-clay-50 text-ink"
                  : "border-cream-300 bg-cream-50 text-ink-soft hover:border-clay/40 hover:bg-clay-50/40"
              }`}
            >
              <span
                className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[0.65rem] font-medium ${
                  picks[i] === oi
                    ...
                    : "border-cream-300 text-ink-faint"
                }`}
              >
      ...
      <div className="mt-5 flex items-center justify-between gap-3">
        <button
          onClick={() => go(-1)}
          disabled={i === 0}
          className="flex items-center gap-1 rounded-lg px-3 py-2 text-sm text-ink-muted transition-colors hover:bg-cream-200 hover:text-ink disabled:pointer-events-none disabled:opacity-40"
        >
        ...
        {i === questions.length - 1 ? (
          <button
            ...
            className="rounded-xl bg-ink px-5 py-2.5 text-sm font-medium text-cream-50 transition-opacity hover:opacity-90"
          >
            {confirmSubmit
          ...
          <button
            onClick={() => go(1)}
            className="flex items-center gap-1 rounded-xl bg-ink px-4 py-2.5 text-sm font-medium text-cream-50 transition-opacity hover:opacity-90"
          >
    ...
    <div>
      <div
        className={`mb-6 rounded-2xl border p-7 text-center shadow-sm ${
          passed ? "border-sage/40 bg-cream-200" : "border-slateblue/40 bg-cream-200"
        }`}
      >
        <div className="mb-1 font-serif text-5xl font-semibold text-ink">{pct}%</div>
          ...
          return (
            <li key={m.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="h-1.5 w-24 shrink-0 overflow-hidden rounded-full bg-cream-300">
                <div
                  className={`h-full rounded-full ${mp >= PASS_MARK * 100 ? "bg-sage" : "bg-slateblue"}`}
                  style={{ width: `${mp}%` }}
                />
              </div>
      ...
      {missed.length > 0 && (
        <>
          <h2 className="mb-3 flex items-center gap-2 font-serif text-lg font-semibold text-ink">
            <AlertTriangle size={17} className="text-slateblue" />
          ...
          <div className="mb-8 space-y-4">
            {missed.map(({ q, pick, n }) => (
              <div
                key={n}
                className="rounded-2xl border border-cream-300 bg-cream-50 p-5"
              >
                <p className="mb-3 font-medium leading-snug text-ink">{q.q}</p>
                {pick !== null && (
                  <p className="mb-1.5 flex items-start gap-2 text-sm text-slateblue">
                    <X size={15} className="mt-0.5 shrink-0" />
                    {q.options[pick]}
                  </p>
                )}
                <p className="mb-3 flex items-start gap-2 text-sm text-sage">
                ...
                <Link
                  href={`/learn/${q.slug}`}
                  className="inline-flex items-center gap-1.5 text-sm text-ink-muted transition-colors hover:text-clay-dark"
                >
