...
export function ReviewDeck({ queue, onGrade, onExit }: Props) {
  ...
  if (done) {
    return (
      <div className="rounded-2xl border border-cream-300 bg-cream-50 p-8 text-center shadow-sm">
        ...
        <div className="mx-auto mb-7 flex max-w-sm justify-between gap-3 text-sm">
          <Tally label="Missed" value={tally.again} tone="text-slateblue" />
          <Tally label="Good" value={tally.good} tone="text-clay-dark" />
          <Tally label="Easy" value={tally.easy} tone="text-sage" />
        </div>
        <button
          onClick={onExit}
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between text-sm text-ink-muted">
        ...
        <button
          onClick={onExit}
          className="flex items-center gap-1 rounded-lg px-2 py-1 transition-colors hover:bg-cream-200 hover:text-ink"
        >
          <X size={14} /> End session
        </button>
      </div>

      <div className="mb-5 h-1.5 overflow-hidden rounded-full bg-cream-300">
        <div
      ...
      >
        ...
        >
          ...
          {flipped ? (
            <p className="mt-5 border-t border-clay/25 pt-5 text-[0.98rem] leading-relaxed text-ink-soft">
              {card.card.back}
            </p>
          ) : (
      ...
      {flipped ? (
        <div className="mt-5 grid grid-cols-3 gap-3">
          <GradeButton
            ...
            className="border-slateblue/40 bg-cream-200 text-slateblue hover:bg-cream-200/70"
          />
          <GradeButton
            label="Good"
            hint="2"
            onClick={() => answer("good")}
            className="border-clay/40 bg-clay-50 text-clay-dark hover:bg-clay-50/70"
          />
          <GradeButton
            label="Easy"
            hint="3"
            onClick={() => answer("easy")}
            className="border-sage/40 bg-cream-200 text-sage hover:bg-cream-200/70"
          />
        </div>
      ) : (
        <button
          onClick={() => setFlipped(true)}
          className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-ink py-3 text-sm font-medium text-cream-50 transition-opacity hover:opacity-90"
        >
      ...
      <Link
        ...
        className="mt-4 inline-flex items-center gap-1.5 text-sm text-ink-muted transition-colors hover:text-clay-dark"
      >
