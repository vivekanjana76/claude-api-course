...
/** Built once on the client — the curriculum and glossary are static. */
...
export function CommandPalette() {
  ...
  return (
    ...
    >
      ...
      />
      <div className="relative w-full max-w-xl rounded-2xl bg-cream-50 shadow-2xl ring-1 ring-cream-300 overflow-hidden animate-[fade-up_0.18s_ease-out]">
        <div className="flex items-center gap-3 px-4 border-b border-cream-300">
          ...
          <input
            ...
            spellCheck={false}
          />
          <kbd className="hidden sm:block text-[0.65rem] font-medium text-ink-muted border border-cream-300 rounded px-1.5 py-0.5">
            ESC
          </kbd>
        </div>
              ...
              <li key={`${item.kind}-${item.href}-${item.title}`}>
                <button
                  ...
                  className={`w-full text-left flex items-center gap-3 px-4 py-2.5 transition-colors ${
                    isActive ? "bg-clay-50" : "hover:bg-cream-200"
                  }`}
                >
                  ...
                  {isActive && (
                    <CornerDownLeft size={14} className="shrink-0 text-clay" />
                  )}
                </button>
              </li>
            );
        ...
        <div className="flex items-center justify-between gap-4 border-t border-cream-300 px-4 py-2 text-[0.68rem] text-ink-muted">
          <span className="flex items-center gap-1.5">
            <kbd className="border border-cream-300 rounded px-1">↑</kbd>
            <kbd className="border border-cream-300 rounded px-1">↓</kbd>
            to navigate
          </span>
          <span className="flex items-center gap-1.5">
