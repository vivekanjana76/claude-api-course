      ...
      {attempts.length > 0 && (
        <>
          <div className="mb-3 flex items-baseline justify-between">
            <h2 className="font-serif text-lg font-semibold text-ink">Past attempts</h2>
            {best && (
          ...
          <ul className="mb-4 divide-y divide-cream-300 overflow-hidden rounded-2xl border border-cream-300 bg-cream-50">
            {attempts.map((a) => {
              ...
              return (
                <li key={a.at} className="flex items-center gap-3 px-4 py-3">
                  <span
                    className={`w-12 shrink-0 font-serif text-lg font-semibold tabular-nums ${
                      passed ? "text-sage" : "text-slateblue"
                    }`}
                  >
                    {pct}%
                  </span>
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-sm transition-colors ${
        active ? "bg-ink text-cream-50" : "bg-cream-200 text-ink-soft hover:bg-cream-300"
      }`}
    >
