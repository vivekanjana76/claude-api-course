...
// The AWS service landscape: the categories a cloud engineer actually lives in.
function AwsServiceMap() {
  const cats = [
    { t: "Compute", s: "EC2 · Lambda · ECS/EKS", c: C.iris },
    { t: "Storage", s: "S3 · EBS · EFS · Glacier", c: C.amber },
    { t: "Networking", s: "VPC · Route 53 · CloudFront", c: C.teal },
    { t: "Database", s: "RDS · Aurora · DynamoDB", c: C.iris },
    { t: "Security", s: "IAM · KMS · GuardDuty", c: C.rose },
    { t: "Ops & IaC", s: "CloudWatch · CFN · SSM", c: C.teal },
  ];
  return (
    <Frame h={330}>
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
        300+ services — but a cloud engineer works in six neighbourhoods
      </text>
      {cats.map((c, i) => {
        const col = i % 3;
        ...
        return (
          <g key={i}>
            <rect x={x} y={y} width={220} height={96} rx={14} fill={C.card} stroke={c.c} strokeWidth={1.8} />
            <rect x={x} y={y} width={220} height={6} rx={3} fill={c.c} />
            <text x={x + 110} y={y + 44} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={16} fontWeight={700} fill={c.c}>
              {c.t}
            </text>
            <text x={x + 110} y={y + 70} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>
              {c.s}
            </text>
          </g>
        );
      })}
      <Cap x={400} y={318} text="Learn these six deeply and the rest of the catalog becomes variations on a theme." />
    </Frame>
  );
}
...
function ComputeSpectrum() {
  const stops = [
    ...
    { t: "Virtual machine", s: "EC2 instance", c: C.amber },
    { t: "Container", s: "ECS / EKS task", c: C.teal },
    { t: "Serverless", s: "Lambda function", c: C.iris },
  ];
  const w = 165,
    gap = 15,
    x0 = 45;
...
function StorageTypes() {
  const kinds = [
    { t: "Object", s: "Amazon S3", d: "flat buckets · HTTP API · massive scale", c: C.iris, ex: "media, backups, data lakes" },
    { t: "Block", s: "EBS / Managed Disk", d: "raw volume attached to one VM", c: C.amber, ex: "boot disks, databases" },
    { t: "File", s: "Amazon EFS / FSx", d: "shared POSIX/SMB mount", c: C.teal, ex: "shared app files, lift-and-shift" },
  ];
  return (
    <Frame h={280}>
      {kinds.map((k, i) => {
...
function Cdn() {
  ...
  return (
    <Frame h={300}>
      ...
      <Cap x={400} y={284} text="CloudFront caches at 600+ edge PoPs near users; only cache misses hit the origin." />
    </Frame>
  );
}
...
function DnsResolution() {
  const steps = [
    ...
    { t: "Amazon Route 53", s: "authoritative" },
...
function WellArchitected() {
  ...
  return (
    <Frame h={230}>
      ...
      <Cap x={400} y={200} text="The AWS Well-Architected Framework: six lenses to review any design against." />
    </Frame>
  );
}
...
function DatabaseTypes() {
  const nosql = [
    ...
    { t: "Document", s: "DynamoDB · DocumentDB", d: "flexible JSON documents" },
    { t: "Wide-column", s: "Cassandra · Bigtable", d: "huge write-heavy tables" },
    { t: "Graph", s: "Amazon Neptune", d: "relationships & traversal" },
  ];
  return (
    <Frame h={300}>
      {/* relational */}
      ...
      <text x={160} y={104} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>RDS · Aurora · Redshift</text>
...
function ServerlessEvent() {
  ...
  return (
    <Frame h={250}>
      ...
      <text x={405} y={138} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>AWS Lambda</text>
...
function ContainerOrchestration() {
  return (
    <Frame h={280}>
      ...
      <text x={400} y={73} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>ECS / EKS — schedules, heals, scales</text>
...
/* ---------- AWS-specific diagrams ---------- */

// A multi-account AWS Organization with OUs and guardrails.
function MultiAccountOrg() {
  const ous = [
    { t: "Security OU", s: "log archive · audit", c: C.rose },
    { t: "Workloads OU", s: "prod · staging · dev", c: C.iris },
    { t: "Sandbox OU", s: "experiments", c: C.amber },
  ];
  return (
    <Frame h={330}>
      <rect x={290} y={40} width={220} height={54} rx={12} fill={C.irisSoft} stroke={C.iris} strokeWidth={2} />
      <text x={400} y={64} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={14} fontWeight={700} fill={C.iris}>Management account</text>
      <text x={400} y={82} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>Organizations · SCPs · consolidated billing</text>
      {ous.map((o, i) => {
        const x = 55 + i * 245;
        return (
          <g key={i}>
            <Arrow x1={400} y1={96} x2={x + 110} y2={135} color={C.line} />
            <rect x={x} y={140} width={220} height={64} rx={12} fill={C.card} stroke={o.c} strokeWidth={1.7} />
            <text x={x + 110} y={166} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13.5} fontWeight={700} fill={o.c}>{o.t}</text>
            <text x={x + 110} y={186} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>{o.s}</text>
            {[0, 1].map((k) => (
              <g key={k}>
                <rect x={x + 20 + k * 110} y={218} width={90} height={38} rx={8} fill={C.canvas} stroke={C.line} strokeWidth={1.2} />
                <text x={x + 65 + k * 110} y={242} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10} fill={C.soft}>account</text>
              </g>
            ))}
          </g>
        );
      })}
      <rect x={55} y={274} width={690} height={30} rx={8} fill={C.roseSoft} stroke={C.rose} strokeWidth={1.3} />
      <text x={400} y={294} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fontWeight={600} fill={C.rose}>
        Service Control Policies set the maximum permissions every account below can ever have
      </text>
      <Cap x={400} y={322} text="The account is the blast radius. Separate prod from dev with accounts, not just tags." />
    </Frame>
  );
}
...
function ElbFamily() {
  const rows = [
    { t: "Application LB", l: "Layer 7 (HTTP/S)", u: "path & host routing, WAF, OIDC auth", c: C.iris },
    { t: "Network LB", l: "Layer 4 (TCP/UDP)", u: "extreme throughput, static IP, low latency", c: C.teal },
    { t: "Gateway LB", l: "Layer 3 (IP)", u: "inline firewalls & security appliances", c: C.rose },
  ];
  return (
    <Frame h={280}>
      {rows.map((r, i) => {
        ...
        return (
          <g key={i}>
            <rect x={45} y={y} width={710} height={60} rx={12} fill={C.card} stroke={r.c} strokeWidth={1.7} />
            <rect x={45} y={y} width={6} height={60} rx={3} fill={r.c} />
            <text x={70} y={y + 26} fontFamily="var(--font-sans)" fontSize={15} fontWeight={700} fill={r.c}>{r.t}</text>
            <text x={70} y={y + 46} fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>{r.l}</text>
            <text x={400} y={y + 36} fontFamily="var(--font-sans)" fontSize={12.5} fill={C.soft}>{r.u}</text>
          </g>
        );
      })}
      <Cap x={400} y={268} text="Pick by the layer you need to make decisions at — HTTP content, raw TCP, or transparent inspection." />
    </Frame>
  );
}

// EC2 purchase options plotted on price vs commitment.
function Ec2PurchaseOptions() {
  const opts = [
    { t: "Spot", d: "up to 90% off", n: "interruptible", c: C.amber, x: 90 },
    { t: "Savings Plans", d: "up to 72% off", n: "1–3 yr commit", c: C.teal, x: 285 },
    { t: "Reserved", d: "up to 72% off", n: "capacity reservation", c: C.iris, x: 480 },
    { t: "On-demand", d: "list price", n: "zero commitment", c: C.muted, x: 675 },
  ];
  return (
    <Frame h={280}>
      <line x1={45} y1={210} x2={755} y2={210} stroke={C.line} strokeWidth={1.5} />
      <text x={60} y={234} fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>← cheaper, less flexible</text>
      <text x={740} y={234} textAnchor="end" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>more flexible, pricier →</text>
      {opts.map((o, i) => (
        <g key={i}>
          <rect x={o.x - 82} y={70} width={164} height={110} rx={13} fill={C.card} stroke={o.c} strokeWidth={1.8} />
          <text x={o.x} y={104} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={15} fontWeight={700} fill={o.c}>{o.t}</text>
          <text x={o.x} y={128} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.soft}>{o.d}</text>
          <text x={o.x} y={152} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fontStyle="italic" fill={C.muted}>{o.n}</text>
          <circle cx={o.x} cy={210} r={5} fill={o.c} />
          <line x1={o.x} y1={180} x2={o.x} y2={205} stroke={o.c} strokeWidth={1.3} strokeDasharray="3 3" />
        </g>
      ))}
      <Cap x={400} y={266} text="Baseline on Savings Plans, burst on on-demand, run fault-tolerant batch on Spot." />
    </Frame>
  );
}

// How an S3 GET is authorized and served.
function S3RequestFlow() {
  return (
    <Frame h={300}>
      <Node x={40} y={120} w={140} h={62} label="Client" sub="SDK / HTTPS" />
      <Arrow x1={182} y1={151} x2={248} y2={151} flow />
      <rect x={250} y={60} width={300} height={185} rx={14} fill={C.irisSoft} stroke={C.iris} strokeWidth={1.8} />
      <text x={400} y={86} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13.5} fontWeight={700} fill={C.iris}>Amazon S3 authorization</text>
      {[
        "1 · Block Public Access — hard stop",
        "2 · Bucket policy (resource)",
        "3 · IAM identity policy",
        "4 · SCP / VPC endpoint policy",
        "5 · Object ownership & ACL",
      ].map((t, i) => (
        <g key={i}>
          <rect x={272} y={100 + i * 28} width={256} height={24} rx={6} fill={C.card} stroke={C.line} strokeWidth={1.1} />
          <text x={284} y={117 + i * 28} fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>{t}</text>
        </g>
      ))}
      <Arrow x1={552} y1={151} x2={618} y2={151} flow />
      <Node x={620} y={120} w={140} h={62} label="Object" sub="bytes + metadata" accent />
      <Cap x={400} y={280} text="An explicit Deny anywhere in the chain wins. Public access is off by default — keep it that way." />
    </Frame>
  );
}

// Route 53 routing policies.
function Route53Routing() {
  const pol = [
    { t: "Simple", d: "one answer", c: C.muted },
    { t: "Weighted", d: "% split — canary", c: C.iris },
    { t: "Latency", d: "fastest region", c: C.teal },
    { t: "Failover", d: "primary → standby", c: C.rose },
    { t: "Geolocation", d: "by user country", c: C.amber },
    { t: "Multivalue", d: "several healthy IPs", c: C.iris },
  ];
  return (
    <Frame h={300}>
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
        Route 53 decides which answer a DNS query gets
      </text>
      {pol.map((p, i) => {
        const x = 45 + (i % 3) * 245;
        const y = 62 + Math.floor(i / 3) * 100;
        return (
          <g key={i}>
            <rect x={x} y={y} width={220} height={78} rx={13} fill={C.card} stroke={p.c} strokeWidth={1.7} />
            <text x={x + 110} y={y + 34} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={15} fontWeight={700} fill={p.c}>{p.t}</text>
            <text x={x + 110} y={y + 58} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>{p.d}</text>
          </g>
        );
      })}
      <Cap x={400} y={286} text="Health checks can be attached to any policy so unhealthy endpoints stop being returned." />
    </Frame>
  );
}

// Ways to connect on-prem to AWS.
function HybridConnectivity() {
  const opts = [
    { t: "Site-to-Site VPN", d: "IPsec over the internet", n: "minutes to set up · variable latency", c: C.teal },
    { t: "Direct Connect", d: "private fibre circuit", n: "weeks to provision · consistent latency", c: C.iris },
    { t: "Transit Gateway", d: "hub for VPCs + on-prem", n: "replaces peering mesh at scale", c: C.amber },
  ];
  return (
    <Frame h={300}>
      <Node x={40} y={110} w={150} h={80} label="On-prem" sub="data center" />
      {opts.map((o, i) => {
        const y = 55 + i * 78;
        return (
          <g key={i}>
            <rect x={250} y={y} width={330} height={62} rx={12} fill={C.card} stroke={o.c} strokeWidth={1.7} />
            <text x={268} y={y + 26} fontFamily="var(--font-sans)" fontSize={14} fontWeight={700} fill={o.c}>{o.t}</text>
            <text x={268} y={y + 45} fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>{o.d} · {o.n}</text>
            <Arrow x1={192} y1={150} x2={246} y2={y + 31} color={C.line} />
            <Arrow x1={582} y1={y + 31} x2={636} y2={150} color={C.line} />
          </g>
        );
      })}
      <Node x={640} y={110} w={120} h={80} label="VPC" sub="in AWS" accent />
      <Cap x={400} y={288} text="Many teams run Direct Connect with a VPN backup — private speed plus an encrypted fallback." />
    </Frame>
  );
}

// DynamoDB partition + sort key model.
function DynamoDbKeys() {
  const items = [
    ["USER#12", "PROFILE", "name, email"],
    ["USER#12", "ORDER#901", "total, status"],
    ["USER#12", "ORDER#902", "total, status"],
    ["USER#77", "PROFILE", "name, email"],
  ];
  return (
    <Frame h={300}>
      <text x={165} y={48} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={12.5} fontWeight={700} fill={C.iris}>Partition key (PK)</text>
      <text x={400} y={48} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={12.5} fontWeight={700} fill={C.teal}>Sort key (SK)</text>
      <text x={640} y={48} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={12.5} fontWeight={700} fill={C.muted}>Attributes</text>
      {items.map((it, i) => {
        const y = 64 + i * 46;
        return (
          <g key={i}>
            <rect x={45} y={y} width={240} height={38} rx={8} fill={C.irisSoft} stroke={C.iris} strokeWidth={1.2} />
            <text x={165} y={y + 24} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={11.5} fill={C.ink}>{it[0]}</text>
            <rect x={295} y={y} width={210} height={38} rx={8} fill={C.tealSoft} stroke={C.teal} strokeWidth={1.2} />
            <text x={400} y={y + 24} textAnchor="middle" fontFamily="var(--font-mono)" fontSize={11.5} fill={C.ink}>{it[1]}</text>
            <rect x={515} y={y} width={240} height={38} rx={8} fill={C.card} stroke={C.line} strokeWidth={1.2} />
            <text x={635} y={y + 24} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.muted}>{it[2]}</text>
          </g>
        );
      })}
      <Cap x={400} y={272} text="Same PK = same partition, stored together and sorted by SK — one query returns a user and all their orders." />
    </Frame>
  );
}

// How IAM decides allow or deny.
function IamPolicyEvaluation() {
  const steps = [
    { t: "Explicit DENY anywhere?", r: "→ DENY", c: C.rose },
    { t: "SCP / permission boundary allows?", r: "no → DENY", c: C.amber },
    { t: "Identity or resource policy ALLOWs?", r: "no → DENY", c: C.teal },
    { t: "Otherwise", r: "→ ALLOW", c: C.iris },
  ];
  return (
    <Frame h={300}>
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
      ...
      {steps.map((s, i) => {
        ...
        return (
          <g key={i}>
            <rect x={90} y={y} width={470} height={44} rx={11} fill={C.card} stroke={s.c} strokeWidth={1.7} />
            <text x={112} y={y + 28} fontFamily="var(--font-sans)" fontSize={13} fill={C.soft}>{s.t}</text>
            <text x={600} y={y + 28} fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={s.c}>{s.r}</text>
            {i < steps.length - 1 && <Arrow x1={325} y1={y + 44} x2={325} y2={y + 56} color={C.line} />}
          </g>
        );
      })}
      <Cap x={400} y={288} text="Deny always beats allow. Nothing is permitted unless something explicitly allows it." />
    </Frame>
  );
}

// KMS envelope encryption.
function KmsEnvelope() {
  return (
    <Frame h={290}>
      <Node x={40} y={110} w={150} h={70} label="Plaintext" sub="your data" />
      <Arrow x1={192} y1={145} x2={252} y2={145} flow />
      <rect x={255} y={95} width={190} height={100} rx={13} fill={C.amberSoft} stroke={C.amber} strokeWidth={1.8} />
      <text x={350} y={126} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13.5} fontWeight={700} fill={C.amber}>Data key (DEK)</text>
      <text x={350} y={150} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>encrypts the data locally</text>
      <text x={350} y={174} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>fast, never leaves the host</text>
      <Arrow x1={447} y1={145} x2={507} y2={145} flow />
      <rect x={510} y={95} width={250} height={100} rx={13} fill={C.irisSoft} stroke={C.iris} strokeWidth={1.8} />
      <text x={635} y={126} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13.5} fontWeight={700} fill={C.iris}>KMS key (CMK)</text>
      <text x={635} y={150} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>encrypts the data key</text>
      <text x={635} y={174} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>audited in CloudTrail · rotatable</text>
      <text x={400} y={230} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={12} fill={C.muted}>
        Stored object = encrypted data + encrypted data key, side by side
      </text>
      <Cap x={400} y={266} text="Envelope encryption keeps bulk crypto local and fast while the master key stays inside KMS." />
    </Frame>
  );
}

// Lambda invocation lifecycle & cold start.
function LambdaLifecycle() {
  const phases = [
    { t: "Download code", d: "cold only", c: C.rose },
    { t: "Start runtime", d: "cold only", c: C.rose },
    { t: "Init code", d: "outside handler", c: C.amber },
    { t: "Invoke handler", d: "every request", c: C.iris },
  ];
  return (
    <Frame h={280}>
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
        Cold start = the first three phases; warm invocations skip straight to the handler
      </text>
      {phases.map((p, i) => {
        const x = 45 + i * 185;
        return (
          <g key={i}>
            <rect x={x} y={80} width={165} height={90} rx={13} fill={C.card} stroke={p.c} strokeWidth={1.8} />
            <text x={x + 82} y={118} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13.5} fontWeight={700} fill={p.c}>{p.t}</text>
            <text x={x + 82} y={142} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.muted}>{p.d}</text>
            {i < 3 && <Arrow x1={x + 168} y1={125} x2={x + 182} y2={125} color={C.line} />}
          </g>
        );
      })}
      <rect x={45} y={192} width={525} height={26} rx={7} fill={C.roseSoft} stroke={C.rose} strokeWidth={1.2} />
      <text x={307} y={210} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fontWeight={600} fill={C.rose}>cold start latency</text>
      <rect x={585} y={192} width={170} height={26} rx={7} fill={C.irisSoft} stroke={C.iris} strokeWidth={1.2} />
      <text x={670} y={210} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fontWeight={600} fill={C.iris}>billed duration</text>
      <Cap x={400} y={252} text="Reuse clients in the init phase; provisioned concurrency removes cold starts for latency-critical paths." />
    </Frame>
  );
}

// SNS fan-out to multiple SQS queues / Lambdas.
function EventDrivenFanout() {
  const targets = [
    { t: "SQS → billing", c: C.teal },
    { t: "SQS → analytics", c: C.amber },
    { t: "Lambda → email", c: C.iris },
  ];
  return (
    <Frame h={300}>
      <Node x={35} y={125} w={140} h={64} label="Producer" sub="order service" />
      <Arrow x1={177} y1={157} x2={273} y2={157} flow />
      <rect x={275} y={120} width={170} height={74} rx={13} fill={C.irisSoft} stroke={C.iris} strokeWidth={2} />
      <text x={360} y={152} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={14} fontWeight={700} fill={C.iris}>SNS topic</text>
      <text x={360} y={174} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>or EventBridge bus</text>
      {targets.map((t, i) => {
        const y = 62 + i * 82;
        return (
          <g key={i}>
            <Arrow x1={447} y1={157} x2={545} y2={y + 28} color={t.c} flow />
            <rect x={550} y={y} width={210} height={56} rx={12} fill={C.card} stroke={t.c} strokeWidth={1.7} />
            <text x={655} y={y + 34} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={600} fill={t.c}>{t.t}</text>
          </g>
        );
      })}
      <Cap x={400} y={288} text="The producer knows nothing about consumers — add a subscriber without touching the publisher." />
    </Frame>
  );
}

// SQS decoupling producer from consumer.
function QueueDecoupling() {
  return (
    <Frame h={290}>
      <Node x={35} y={110} w={150} h={70} label="Producer" sub="web tier" />
      <Arrow x1={187} y1={145} x2={247} y2={145} flow />
      <rect x={250} y={100} width={280} height={90} rx={13} fill={C.tealSoft} stroke={C.teal} strokeWidth={1.9} />
      <text x={390} y={126} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={14} fontWeight={700} fill={C.teal}>SQS queue</text>
      {[0, 1, 2, 3, 4].map((i) => (
        <rect key={i} x={274 + i * 50} y={142} width={40} height={30} rx={6} fill={C.card} stroke={C.teal} strokeWidth={1.2} />
      ))}
      <Arrow x1={532} y1={145} x2={592} y2={145} flow />
      <Node x={595} y={100} w={165} h={40} label="Worker" />
      <Node x={595} y={150} w={165} h={40} label="Worker" />
      <text x={390} y={218} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>
        buffer absorbs spikes · retries on failure · DLQ catches poison messages
      </text>
      <Cap x={400} y={258} text="Queues turn a synchronous dependency into an asynchronous one — the web tier stays up when workers stall." />
    </Frame>
  );
}
...
function EcsVsEks() {
  const cols = [
    { t: "ECS", s: "AWS-native orchestrator", pts: ["Simplest path on AWS", "Task definitions, not YAML sprawl", "Deep IAM/ALB/CW integration", "No control-plane fee"], c: C.iris },
    { t: "EKS", s: "managed Kubernetes", pts: ["Portable, standard K8s API", "Huge ecosystem (Helm, operators)", "Steeper learning curve", "Control-plane hourly fee"], c: C.teal },
  ];
  return (
    <Frame h={300}>
      {cols.map((c, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={45} width={330} height={200} rx={14} fill={C.card} stroke={c.c} strokeWidth={1.9} />
            <text x={x + 165} y={80} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={19} fontWeight={700} fill={c.c}>{c.t}</text>
            <text x={x + 165} y={102} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>{c.s}</text>
            {c.pts.map((p, j) => (
              <text key={j} x={x + 24} y={132 + j * 26} fontFamily="var(--font-sans)" fontSize={11.5} fill={C.soft}>• {p}</text>
            ))}
          </g>
        );
      })}
      <Cap x={400} y={278} text="Both run on Fargate if you don't want to manage nodes. Choose ECS for speed, EKS for portability." />
    </Frame>
  );
}

// A CI/CD pipeline on AWS.
function CicdPipeline() {
  const stages = [
    { t: "Source", d: "Git push", c: C.muted },
    { t: "Build", d: "CodeBuild", c: C.teal },
    { t: "Test", d: "unit + scan", c: C.amber },
    { t: "Approve", d: "manual gate", c: C.rose },
    { t: "Deploy", d: "CodeDeploy / CFN", c: C.iris },
  ];
  return (
    <Frame h={260}>
      {stages.map((s, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={80} width={135} height={82} rx={13} fill={C.card} stroke={s.c} strokeWidth={1.8} />
            <text x={x + 67} y={116} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={14} fontWeight={700} fill={s.c}>{s.t}</text>
            <text x={x + 67} y={138} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>{s.d}</text>
            {i < stages.length - 1 && <Arrow x1={x + 138} y1={121} x2={x + 150} y2={121} color={C.line} flow />}
          </g>
        );
      })}
      <Cap x={400} y={200} text="Every commit takes the same path to production — the pipeline is the only way in." />
      <Cap x={400} y={224} text="Artifacts are built once and promoted; environments differ only by configuration." />
    </Frame>
  );
}
...
function DeploymentStrategies() {
  const rows = [
    { t: "Rolling", d: "replace instances a batch at a time", r: "cheap · slow rollback", c: C.teal },
    { t: "Blue / green", d: "stand up a full new fleet, flip traffic", r: "instant rollback · 2× cost briefly", c: C.iris },
    { t: "Canary", d: "send 5% of traffic, watch, then ramp", r: "smallest blast radius · needs good metrics", c: C.amber },
  ];
  return (
    <Frame h={280}>
      {rows.map((r, i) => {
        ...
        return (
          <g key={i}>
            ...
            <rect x={45} y={y} width={6} height={60} rx={3} fill={r.c} />
            <text x={70} y={y + 26} fontFamily="var(--font-sans)" fontSize={15} fontWeight={700} fill={r.c}>{r.t}</text>
            <text x={70} y={y + 46} fontFamily="var(--font-sans)" fontSize={11.5} fill={C.muted}>{r.d}</text>
            <text x={470} y={y + 36} fontFamily="var(--font-sans)" fontSize={12} fontStyle="italic" fill={C.soft}>{r.r}</text>
          </g>
        );
      })}
      <Cap x={400} y={268} text="All three need health checks that actually fail when the app is broken — otherwise you automate an outage." />
    </Frame>
  );
}

// The main levers for reducing an AWS bill.
function CostLevers() {
  const levers = [
    { t: "Rightsize", d: "smaller instances, gp3 not io1", c: C.iris },
    { t: "Commit", d: "Savings Plans on steady baseline", c: C.teal },
    { t: "Schedule", d: "stop dev at night & weekends", c: C.amber },
    { t: "Tier storage", d: "lifecycle to IA / Glacier", c: C.iris },
    { t: "Kill waste", d: "idle EBS, old snapshots, stray NATs", c: C.rose },
    { t: "Cut egress", d: "CloudFront, VPC endpoints, same-AZ", c: C.teal },
  ];
  return (
    <Frame h={320}>
      {levers.map((l, i) => {
        const x = 45 + (i % 3) * 245;
        const y = 50 + Math.floor(i / 3) * 108;
        return (
          <g key={i}>
            <rect x={x} y={y} width={220} height={86} rx={13} fill={C.card} stroke={l.c} strokeWidth={1.7} />
            <text x={x + 110} y={y + 36} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={15} fontWeight={700} fill={l.c}>{l.t}</text>
            <text x={x + 110} y={y + 62} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.muted}>{l.d}</text>
          </g>
        );
      })}
      <Cap x={400} y={296} text="Tag everything first — you cannot optimize a bill you cannot attribute to a team or service." />
    </Frame>
  );
}

// The four disaster-recovery strategies, ordered by RTO/cost.
function DrStrategies() {
  const strat = [
    { t: "Backup & restore", r: "RTO: hours", c: "$", col: C.muted },
    { t: "Pilot light", r: "RTO: tens of min", c: "$$", col: C.amber },
    { t: "Warm standby", r: "RTO: minutes", c: "$$$", col: C.teal },
    { t: "Multi-site active/active", r: "RTO: near zero", c: "$$$$", col: C.iris },
  ];
  return (
    <Frame h={290}>
      {strat.map((s, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={200 - h} width={172} height={h} rx={12} fill={s.col} opacity={0.16} stroke={s.col} strokeWidth={1.8} />
            <text x={x + 86} y={228} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={s.col}>{s.t}</text>
            <text x={x + 86} y={248} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.muted}>{s.r} · {s.c}</text>
          </g>
        );
      })}
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
        Faster recovery costs more — pick the cheapest option that meets the business RTO/RPO
      </text>
      <Cap x={400} y={276} text="A DR plan you have never tested is a hypothesis, not a plan." />
    </Frame>
  );
}

// The 7 Rs of migration.
function Migration7Rs() {
  const rs = [
    { t: "Retire", d: "switch it off", c: C.muted },
    { t: "Retain", d: "leave on-prem for now", c: C.muted },
    { t: "Rehost", d: "lift & shift to EC2", c: C.teal },
    { t: "Relocate", d: "move VMware wholesale", c: C.teal },
    { t: "Replatform", d: "lift & tinker (RDS, ALB)", c: C.amber },
    { t: "Repurchase", d: "buy SaaS instead", c: C.rose },
    { t: "Refactor", d: "rebuild cloud-native", c: C.iris },
  ];
  return (
    <Frame h={300}>
      <text x={400} y={40} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={13} fontWeight={700} fill={C.muted}>
        Seven ways to move a workload — effort rises left to right
      </text>
      {rs.map((r, i) => {
        const x = 45 + (i % 4) * 185;
        const y = 62 + Math.floor(i / 4) * 105;
        return (
          <g key={i}>
            <rect x={x} y={y} width={165} height={82} rx={13} fill={C.card} stroke={r.c} strokeWidth={1.7} />
            <text x={x + 82} y={y + 36} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={14.5} fontWeight={700} fill={r.c}>{r.t}</text>
            <text x={x + 82} y={y + 60} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fill={C.muted}>{r.d}</text>
          </g>
        );
      })}
      <Cap x={400} y={288} text="Most portfolios end up mostly rehost/replatform, with refactor reserved for the few apps that earn it." />
    </Frame>
  );
}

// The canonical three-tier AWS reference architecture.
function ThreeTierReference() {
  return (
    <Frame h={340}>
      <Node x={330} y={30} w={140} h={44} label="Route 53" />
      <Arrow x1={400} y1={76} x2={400} y2={96} color={C.line} flow />
      <Node x={330} y={98} w={140} h={44} label="CloudFront" sub="+ WAF" />
      <Arrow x1={400} y1={144} x2={400} y2={164} color={C.line} flow />
      <rect x={45} y={166} width={710} height={140} rx={14} fill="none" stroke={C.iris} strokeWidth={1.6} strokeDasharray="7 6" />
      <text x={62} y={186} fontFamily="var(--font-sans)" fontSize={11.5} fontWeight={700} fill={C.iris}>VPC · two AZs</text>
      <rect x={300} y={166} width={200} height={34} rx={9} fill={C.irisSoft} stroke={C.iris} strokeWidth={1.6} />
      <text x={400} y={188} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={12} fontWeight={700} fill={C.iris}>Application Load Balancer</text>
      {[0, 1].map((i) => {
        const x = 90 + i * 380;
        return (
          <g key={i}>
            <text x={x + 145} y={220} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={10.5} fontWeight={700} fill={C.teal}>
              AZ {String.fromCharCode(97 + i)} · private subnet
            </text>
            <rect x={x} y={228} width={290} height={34} rx={8} fill={C.card} stroke={C.teal} strokeWidth={1.4} />
            <text x={x + 145} y={250} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11.5} fill={C.soft}>App tier — ASG / ECS tasks</text>
            <rect x={x} y={266} width={290} height={32} rx={8} fill={C.tealSoft} stroke={C.teal} strokeWidth={1.4} />
            <text x={x + 145} y={287} textAnchor="middle" fontFamily="var(--font-sans)" fontSize={11} fill={C.soft}>
              {i === 0 ? "RDS primary + ElastiCache" : "RDS standby (Multi-AZ)"}
            </text>
          </g>
        );
      })}
      <Cap x={400} y={328} text="Public edge, private compute, private data — replicated across two AZs. The default shape of most AWS apps." />
    </Frame>
  );
}
...
const REGISTRY: Record<DiagramName, () => JSX.Element> = {
  ...
  "multi-account-org": MultiAccountOrg,
  "elb-family": ElbFamily,
  "ec2-purchase-options": Ec2PurchaseOptions,
  "s3-request-flow": S3RequestFlow,
  "route53-routing": Route53Routing,
  "hybrid-connectivity": HybridConnectivity,
  "dynamodb-keys": DynamoDbKeys,
  "iam-policy-evaluation": IamPolicyEvaluation,
  "kms-envelope": KmsEnvelope,
  "lambda-lifecycle": LambdaLifecycle,
  "event-driven-fanout": EventDrivenFanout,
  "queue-decoupling": QueueDecoupling,
  "ecs-vs-eks": EcsVsEks,
  "cicd-pipeline": CicdPipeline,
  "deployment-strategies": DeploymentStrategies,
  "cost-levers": CostLevers,
  "dr-strategies": DrStrategies,
  "migration-7rs": Migration7Rs,
  "three-tier-reference": ThreeTierReference,
};
