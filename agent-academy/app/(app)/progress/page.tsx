  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12 lg:px-10">
      ...
      <div className="mb-8 rounded-2xl border border-canvas-300 bg-canvas-50 p-6 shadow-sm">
        ...
        <div className="mb-5 h-2 overflow-hidden rounded-full bg-canvas-300">
          <div
            className="h-full rounded-full bg-iris transition-all duration-700"
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      {streak > 0 && (
        <div className="mb-8 flex items-center gap-3 rounded-2xl border border-amber/40 bg-amber-50 px-5 py-4">
          <Flame size={20} className="shrink-0 text-amber-dark" />
      ...
      <h2 className="mb-3 font-display text-lg font-semibold text-ink">Last 13 weeks</h2>
      <div className="mb-8 overflow-x-auto rounded-2xl border border-canvas-300 bg-canvas-50 p-5">
      ...
      <h2 className="mb-3 font-display text-lg font-semibold text-ink">By module</h2>
        ...
        {modules.map((m) => {
          ...
          return (
            <li key={m.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="h-1.5 w-28 shrink-0 overflow-hidden rounded-full bg-canvas-300">
                <div
      ...
      {shaky.length > 0 && (
        <>
          <h2 className="mb-1 font-display text-lg font-semibold text-ink">Worth another pass</h2>
            ...
            {shaky.slice(0, 6).map(({ ref, score }) => (
              <li key={ref.lesson.slug}>
                <Link
                  ...
                  className="group flex items-center gap-3 px-4 py-3 transition-colors hover:bg-canvas-100"
                >
                  <MasteryRing correct={score!.correct} total={score!.total} size={30} />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm text-ink group-hover:text-iris-dark">
                      {ref.lesson.title}
                    </span>
      ...
      {next && (
        <Link
          ...
          className="group mb-10 flex items-center gap-3 rounded-2xl border border-iris/30 bg-canvas-50 px-5 py-4 transition-all hover:border-iris/50 hover:shadow-md"
        >
          <span className="min-w-0 flex-1">
            <span className="block text-xs font-medium uppercase tracking-wider text-iris-dark">
              Up next
            </span>
          ...
          <ArrowRight
            size={18}
            className="shrink-0 text-iris transition-transform group-hover:translate-x-1"
          />
        </Link>
      )}
...
function BackupPanel({ onDownload }: { onDownload: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-canvas-300 bg-canvas-50 p-6">
      <h2 className="mb-1 font-display text-lg font-semibold text-ink">Back up or move</h2>
      ...
      <div className="flex flex-wrap gap-3">
        <button
          onClick={onDownload}
          className="flex items-center gap-2 rounded-xl bg-ink px-4 py-2.5 text-sm font-medium text-canvas-50 transition-opacity hover:opacity-90"
        >
          <Download size={15} /> Export
        </button>
        <button
          onClick={() => fileRef.current?.click()}
          className="flex items-center gap-2 rounded-xl border border-canvas-300 px-4 py-2.5 text-sm font-medium text-ink transition-colors hover:bg-canvas-200"
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-colors ${
            confirming
              ? "border-rose bg-rose-50 text-rose-dark"
              : "border-canvas-300 text-ink-muted hover:text-rose-dark"
          }`}
        >
      ...
      {message && (
        <p className={`mt-4 text-sm ${message.bad ? "text-rose-dark" : "text-teal-dark"}`}>
          {message.text}
        </p>
      )}
    </div>
...
function heatTone(count: number): string {
  if (count === 0) return "bg-canvas-300";
  if (count < 3) return "bg-iris/25";
  if (count < 6) return "bg-iris/50";
  if (count < 12) return "bg-iris/75";
  return "bg-iris";
}
