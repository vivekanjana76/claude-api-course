...
const tabs: { id: TabId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "decoder", label: "Buzzword decoder", icon: <Lightbulb size={15} /> },
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-10 py-12">
      ...
      <p className="text-ink-soft leading-relaxed mb-8">
        ...
        come up most: <em>agent or workflow?</em>, <em>which pattern?</em>,{" "}
        <em>where does the guardrail go?</em>, and <em>what does that buzzword
        actually mean?</em>
      </p>
...
function Decoder() {
  // Turn the buzzword drills into flashcards: prompt front, plain answer back.
  const cards = useMemo(
    () =>
      drills
        .filter((d) => d.skill === "buzzwords")
        .map((d) => ({
          front: d.prompt,
  ...
  return (
    <div>
      <p className="text-ink-soft leading-relaxed mb-6">
        Every hype term maps to a plain engineering idea. Flip each card to
        translate the buzzword you&apos;ll hear in job posts and vendor decks
        into what it actually means on the ground.
      </p>
...
function HeuristicsSection() {
  return (
    <div>
      <p className="text-ink-soft leading-relaxed mb-6">
        Six rules that quietly decide most agent-design questions. The drills
        are just these, dressed up as scenarios.
      </p>
