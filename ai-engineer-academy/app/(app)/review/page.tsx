  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-12 lg:px-10">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat label="Due now" value={stats.due} tone="text-iris-dark" />
        <Stat label="Unseen" value={stats.fresh} tone="text-ink" />
        <Stat label="Learning" value={stats.learning} tone="text-amber-dark" />
        <Stat label="Retained" value={`${stats.retention}%`} tone="text-teal-dark" />
      </div>
      ...
      <div className="mb-8 rounded-2xl border border-canvas-300 bg-canvas-50 p-6 shadow-sm">
        ...
        {queue.length > 0 ? (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-ink py-3 text-sm font-medium text-canvas-50 transition-opacity hover:opacity-90"
          >
            <Play size={15} />
          ...
          <div className="rounded-xl bg-teal-50 px-4 py-5 text-center">
            <Sparkles size={18} className="mx-auto mb-2 text-teal-dark" />
      ...
      <h2 className="mb-3 font-display text-lg font-semibold text-ink">Deck by module</h2>
    ...
    <button
      ...
      className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-sm transition-colors ${
        active
          ...
          : "bg-canvas-200 text-ink-soft hover:bg-canvas-300"
      }`}
    >
...
function Stat({
  ...
  return (
    <div className="rounded-xl border border-canvas-300 bg-canvas-50 px-4 py-3">
