...
export const glossary: GlossaryTerm[] = [
  ...
  { term: "Assertion", def: "A deterministic pass/fail check in an eval suite — parses, matches schema, citations exist, no PII, under budget. Free to run, unambiguous, and the layer most teams skip.", related: ["Eval / evaluation", "Golden set"] },
  ...
  { term: "Circuit breaker", def: "Temporarily stopping traffic to a failing dependency instead of timing out every request against it. Standard resilience pattern at the AI gateway.", related: ["LLM gateway", "Fallback"] },
  ...
  { term: "Cost attribution", def: "Tagging every model call with the tenant, feature, and route that caused it, so spend can be traced to something a business decision can act on. Without it you know the bill went up and nothing else — which is why the gateway is usually where it gets implemented.", related: ["LLM gateway", "Cost per successful outcome"] },
  ...
  { term: "Diffusion model", def: "A generative model trained to reverse a noising process: it starts from noise and denoises step by step into an image, video, or audio clip. The step count is the quality/latency dial, which is why generation is slow relative to a text model and why distilled few-step variants matter in production.", related: ["C2PA", "Deepfake", "Distillation"] },
  ...
  // ── F ──
  { term: "Fallback", def: "An alternative model or provider used when the primary fails. Untested fallbacks aren't fallbacks — run them through the eval suite and exercise them deliberately.", related: ["LLM gateway", "Circuit breaker"] },
  ...
  { term: "Hedged request", def: "Issuing a duplicate request when the first exceeds a latency threshold and taking whichever returns first. Cuts tail latency at the cost of extra tokens.", related: ["LLM gateway", "TTFT"] },
  ...
  { term: "LLMOps", def: "The operational discipline around LLM systems — prompt versioning, tracing, evaluation in CI, cost control, staged rollout, and incident response.", related: ["Observability (LLM)", "Release lifecycle"], hot: true },
  ...
  { term: "NIST AI RMF", def: "A voluntary US AI risk-management framework structured as Govern, Map, Measure, Manage. The common structure for an internal AI risk programme and a defensible security-review answer.", related: ["EU AI Act", "ISO/IEC 42001"], hot: true },
  ...
  { term: "Resumability", def: "The property that an interrupted run can be picked up from its last checkpoint rather than restarted. What turns a long agent run from an all-or-nothing gamble into ordinary work — and the reason a rate limit at step 40 is an inconvenience instead of an outage.", related: ["Checkpointing", "Agent"] },
  ...
  { term: "Voice agent", def: "A conversational agent over speech: transcribe, reason, synthesise, all inside a latency budget of roughly 800ms before a pause sounds like a dropped call. The hard parts are not the model but the turn-taking — endpointing, barge-in, and something to say while thinking.", related: ["Endpointing", "Barge-in", "Backchannel"], hot: true },
  ...
  { term: "EU AI Act", def: "Risk-tiered EU regulation (prohibited / high-risk / limited / minimal). Tier depends on what a system is used for, not how it's built — high-risk carries documentation, human-oversight, accuracy, and logging obligations.", related: ["NIST AI RMF", "ISO/IEC 42001"], hot: true },
