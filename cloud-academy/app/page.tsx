    ...
    <main className="min-h-screen circuit-grid">
      ...
      <header className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Logo size={32} />
          <span className="font-display text-lg font-semibold text-ink tracking-tight">Cloud Academy</span>
        </div>
          ...
          <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-12 items-center">
            <div className="animate-fade-up">
              <div className="inline-flex items-center gap-2 rounded-full border border-iris/30 bg-iris-50 px-3 py-1 text-xs text-iris-dark font-medium mb-6">
                <Sparkles size={13} /> AWS &amp; Azure, side by side — from first principles to production
              </div>
              <h1 className="font-display text-5xl sm:text-6xl font-semibold text-ink leading-[1.05] tracking-tight">
                Master the{" "}
                <span className="text-iris">cloud</span>
                {" "}— on{" "}
                <span className="text-amber-dark">AWS</span>
                {" "}&amp;{" "}
                <span className="text-iris">Azure</span>.
              </h1>
              <p className="mt-6 text-lg text-ink-soft leading-relaxed max-w-xl">
                A beautiful, visual course on cloud computing: compute, storage, networking,
                databases, identity, serverless, and well-architected design — taught for both
                clouds at once, because the concepts are the same and only the names differ.
                No setup, just clear explanations and custom diagrams.
              </p>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <div className="flex items-end justify-between mb-8">
          <div>
            ...
            <p className="text-ink-muted mt-1">Fundamentals first, then real architecture — growing every week.</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6">
            ...
            <div className="flex-1">
              ...
              <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink">
                The Cloud Architecture Pattern Catalog
              </h2>
              <p className="text-ink-soft mt-2 max-w-2xl leading-relaxed">
                {patterns.length} battle-tested patterns — multi-AZ high availability, autoscaling
                web tiers, public/private subnet tiering, edge caching, hub-and-spoke networks and
                more — each with a diagram, when to reach for it, and what to watch out for.
              </p>
            </div>
            <ArrowRight
              size={22}
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        <div className="rounded-3xl bg-ink text-canvas-100 p-10 sm:p-12">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
            <Aid icon={<Network size={18} />} title="Custom diagrams" text="Bespoke visuals for every concept — regions & AZs, autoscaling, VPC layouts, storage tiers, CDNs, and more." />
            ...
            <Aid icon={<MessagesSquare size={18} />} title={`${interviewQA.length} interview Q&As`} text="Model answers to the cloud questions you'll actually be asked — foundations, compute, storage, networking, architecture." />
            <Aid icon={<BookOpen size={18} />} title={`${glossary.length}-term glossary`} text="Searchable definitions for every service and acronym, cross-linked and mapped across AWS and Azure." />
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-canvas-300">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-ink-muted">
          <span>Cloud Academy — a personal learning companion for AWS &amp; Azure.</span>
          <Link href={`/learn/${firstLesson}`} className="text-iris-dark hover:text-iris font-medium flex items-center gap-1">
            Begin with Cloud Foundations <ArrowRight size={14} />
          </Link>
        </div>
      </footer>
    </main>
...
function HeroArt() {
  // A small cloud architecture: users → CDN/LB → app tier across two AZs → data.
  return (
    <div className="relative animate-fade-up" style={{ animationDelay: "0.15s" }}>
      <div className="rounded-3xl border border-canvas-300 bg-canvas-50 p-6 shadow-sm">
        <svg viewBox="0 0 400 360" width="100%">
          {/* region outline */}
          <rect x="30" y="70" width="340" height="250" rx="16" fill="none" stroke="#C4D5E8" strokeWidth="1.5" strokeDasharray="6 6" />
          <text x="46" y="60" fontFamily="var(--font-display)" fontSize="12" fontWeight="600" fill="#2563EB">Region · 2 AZs</text>

          {/* users */}
          <rect x="150" y="12" width="100" height="34" rx="9" fill="#FFFFFF" stroke="#5C6A80" strokeWidth="1.4" />
          <text x="200" y="34" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="12" fontWeight="600" fill="#0F1826">Users</text>

          {/* load balancer */}
          <rect x="140" y="90" width="120" height="42" rx="10" fill="#E7EFFD" stroke="#2563EB" strokeWidth="2" />
          <text x="200" y="116" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="12.5" fontWeight="600" fill="#2563EB">Load balancer</text>
          <line x1="200" y1="46" x2="200" y2="88" stroke="#C4D5E8" strokeWidth="1.5" strokeDasharray="5 5" className="animate-flow" />

          {/* AZ a */}
          <rect x="48" y="160" width="140" height="140" rx="12" fill="#E1F5F9" stroke="#0E9BB5" strokeWidth="1.4" />
          <text x="118" y="182" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11.5" fontWeight="700" fill="#0A7387">AZ-a</text>
          <rect x="68" y="194" width="100" height="38" rx="8" fill="#FFFFFF" stroke="#0E9BB5" strokeWidth="1.3" />
          <text x="118" y="217" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11.5" fill="#0F1826">App VM</text>
          <rect x="68" y="244" width="100" height="38" rx="8" fill="#FFFFFF" stroke="#2563EB" strokeWidth="1.3" />
          <text x="118" y="267" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11" fill="#0F1826">DB primary</text>

          {/* AZ b */}
          <rect x="212" y="160" width="140" height="140" rx="12" fill="#E1F5F9" stroke="#0E9BB5" strokeWidth="1.4" />
          <text x="282" y="182" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11.5" fontWeight="700" fill="#0A7387">AZ-b</text>
          <rect x="232" y="194" width="100" height="38" rx="8" fill="#FFFFFF" stroke="#0E9BB5" strokeWidth="1.3" />
          <text x="282" y="217" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11.5" fill="#0F1826">App VM</text>
          <rect x="232" y="244" width="100" height="38" rx="8" fill="#FFFFFF" stroke="#ED8B00" strokeWidth="1.3" />
          <text x="282" y="267" textAnchor="middle" fontFamily="var(--font-sans)" fontSize="11" fill="#0F1826">DB standby</text>

          {/* LB to app tiers */}
          <line x1="180" y1="132" x2="118" y2="192" stroke="#0E9BB5" strokeWidth="1.5" className="animate-flow" />
          <line x1="220" y1="132" x2="282" y2="192" stroke="#0E9BB5" strokeWidth="1.5" className="animate-flow" />
          {/* db replication */}
          <line x1="168" y1="263" x2="232" y2="263" stroke="#ED8B00" strokeWidth="1.4" strokeDasharray="4 4" />
        </svg>
      </div>
      <div className="absolute -bottom-4 -right-3 rotate-3 rounded-xl bg-canvas-50 border border-canvas-300 px-4 py-2 shadow-md text-xs font-mono text-ink-soft hidden sm:block">
        highly <span className="text-teal-dark">available</span>
      </div>
    </div>
  );
}
